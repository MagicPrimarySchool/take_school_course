...
class Spider:
    class Lesson:
        ...
        def show(self):
            print(
                '  name:' + self.name + '  code:' + self.code + '  teacher_name:' + self.teacher_name + '  time:' + self.time)
    ...
    def login(self, uid, password):
        ...
        while True:
            ...
            try:
                ...
                print('something wrong with the server, wait 0.5 second')
                time.sleep(0.5)
                continue
            if self.__real_base_url != 'http://218.75.197.123:83/':
    ...
    def __get_lessons(self, soup):
        ...
        for lesson_tag in lesson_tag_list:
            ...
            print(code, name, teacher_name, Time, number)
    ...
    def __select_lesson(self, lesson_list):
        ...
        soup = BeautifulSoup(request.text, features="html.parser")
        self.__set__VIEWSTATE(soup)
        error_tag = soup.find_all('script')
...
if __name__ == '__main__':
    ...
    spider = Spider(url)
    start_time = datetime(2019, 3, 8, 17, 59, 30)
    if spider.login(uid, password):
        while datetime.now() < start_time:
            time.sleep(1)
            print('还没到抢课时间哟')
        spider.run(course_name)
    os.system("pause")
